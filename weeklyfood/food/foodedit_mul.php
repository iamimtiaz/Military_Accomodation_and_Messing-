...
<div class="container">
<form method="post" action="foodupdate_mul.php">
...
{
	$id = $chk[$i];			
	$res=$MySQLiconn->query("SELECT * FROM food WHERE id=".$id);
	while($row=$res->fetch_array())
	{
		...
		<input type="text" name="fn[]" value="<?php echo $row['item'];?>" class="form-control" />
        </td>
        <td>
		<input type="text" name="ln[]" value="<?php echo $row['quantity'];?>" class="form-control" />
		</td>
        <td>
		<input type="text" name="r[]" value="<?php echo $row['price'];?>" class="form-control" />
		</td>
		</tr>
		<?php	
	}			
...
<a href="foodindex.php" class="btn btn-large btn-success"> <i class="glyphicon glyphicon-fast-backward"></i> &nbsp; cancel</a>
</td>
</tr>
</table>
</form>
